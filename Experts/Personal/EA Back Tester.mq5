...
// Windows API for pause/resume tester via SPACE key
#import "user32.dll"
...
struct OrderData
{
    ...
    ulong  Deviation;
    ulong  Expiration;
    ulong  OrderStatus;  // 0=no-op | 1=trade | 98=resume | 99=pause
    ulong  Type;
    ulong  TypeTime;
    ulong  Action;
    ulong  TypeFilling;
...
class CClient : public CObject
{
    ...
    void SetOperation(OperationData &operation) { iPipe.WriteStruct(operation); };
    void SetPrice(PriceData &price)             { iPipe.WriteStruct(price);     };
    void SetPosition(PositionData &position)    { iPipe.WriteStruct(position);  };
    void SetCount(CountData &count)             { iPipe.WriteStruct(count);     };
    void GetOrder(OrderData &trade)             { iPipe.ReadStruct(trade);      };
};
...
void SendSpaceKey()
{
    ...
    {
        keybd_event((uchar)VK_SPACE, 0, 0,               0);
        keybd_event((uchar)VK_SPACE, 0, KEYEVENTF_KEYUP, 0);
    }
}
...
{
    long c = 0; // Must be 0 in Strategy Tester, not ChartID()
    ChartSetInteger(c, CHART_MODE,              CHART_CANDLES);
    ChartSetInteger(c, CHART_COLOR_BACKGROUND,  clrBlack);
    ChartSetInteger(c, CHART_COLOR_FOREGROUND,  clrSilver);
    ChartSetInteger(c, CHART_COLOR_CANDLE_BULL, clrDarkGreen);
    ChartSetInteger(c, CHART_COLOR_CHART_UP,    clrDarkGreen);
    ChartSetInteger(c, CHART_COLOR_CANDLE_BEAR, clrMaroon);
    ChartSetInteger(c, CHART_COLOR_CHART_DOWN,  clrMaroon);
    ChartSetInteger(c, CHART_SHOW_GRID,         false);
    ChartSetInteger(c, CHART_SHOW_ASK_LINE,     true);
    ChartSetInteger(c, CHART_COLOR_ASK,         clrDimGray);
    ChartSetInteger(c, CHART_COLOR_BID,         clrSilver);
    ChartSetInteger(c, CHART_SHOW_VOLUMES,      CHART_VOLUME_HIDE);
    ChartSetInteger(c, CHART_SHOW_PERIOD_SEP,   false);
    ChartRedraw(c);
}
...
void OnTick()  { Communication(); }
void OnTimer() { Communication(); }
//+------------------------------------------------------------------+
//| Auto-detect the correct filling mode for this symbol            |
//| FOK and IOC are rejected in the Strategy Tester — use RETURN    |
//+------------------------------------------------------------------+
ENUM_ORDER_TYPE_FILLING GetFillType(string symbol)
{
...
void Communication()
{
    ...
    // FIX: do NOT use = {0} for structs containing enums — use ZeroMemory instead
    PriceData price;
    ZeroMemory(price);
    ...
    // FIX: use ZeroMemory — {0} fails when struct has enum fields in newer MT5
    ...
    // Handle pause/resume signals from panel
    if(order.OrderStatus == 99 && !g_paused)
    {
        SendSpaceKey();
        g_paused = true;
        Print("AIVTester: Tester PAUSED via panel");
    }
    else if(order.OrderStatus == 98 && g_paused)
    {
        SendSpaceKey();
        g_paused = false;
        Print("AIVTester: Tester RESUMED via panel");
    }
    else if(order.OrderStatus > 0)
    {
        ...
        query.type_filling = GetFillType(query.symbol);
        ...
        query.comment      = CharArrayToString(order.Comments, 0, sizeof(order.Comments));
         if((ENUM_SYMBOL_TRADE_MODE)SymbolInfoInteger(query.symbol, SYMBOL_TRADE_MODE) != SYMBOL_TRADE_MODE_FULL)
         {
             Print("AIVTester: Trade not allowed right now for ", query.symbol, " — skipping");
        ...
        StringToCharArray(GetErrorDescription(result.retcode),
                          operation.Message, 0, sizeof(operation.Message));
    }
...
bool FindBestPrice(MqlTradeRequest &query, MqlTradeResult &result, CSymbolInfo &symbolClass)
{
    ...
    if(query.action == TRADE_ACTION_DEAL)
    {
        ...
        {
            query.price = query.type == ORDER_TYPE_BUY
                ? NormalizeDouble(symbolClass.Ask(), _Digits)
                : NormalizeDouble(symbolClass.Bid(), _Digits);
        }
    }
...
bool PositionTrailing()
{
    ...
    {
        // FIX: use PositionGetTicket(i) then SelectByTicket
        // Original PositionGetSymbol(i) without SelectByIndex causes wrong position read
        ulong tk = PositionGetTicket(i);
        if(tk == 0) continue;
        if(!sPos.SelectByTicket(tk)) continue;
        if(sPos.Symbol() != _Symbol) continue;

        if(sPos.PositionType() == POSITION_TYPE_BUY)
        {
            double sl = MathMax(sPos.PriceOpen() + InpBreakeven * _Point,
                                sPos.StopLoss()) + InpTrailingStep * _Point;
        ...
        {
            double sl = MathMin(sPos.PriceOpen() - InpBreakeven * _Point,
                                sPos.StopLoss()) - InpTrailingStep * _Point;
