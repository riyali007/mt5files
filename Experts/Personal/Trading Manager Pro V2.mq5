...
input int    InpDefaultTP  = 1500;      // Default TP (points)
...
input int  InpTrailingStart   = 150;    // Points in profit to activate trailing
input int  InpTrailingStep    = 50;     // Trail distance behind price (points)
...
void OnTick()
{
   ...
   bool priceChanged = (MathAbs(bid - lastBid) >= _Point || MathAbs(ask - lastAsk) >= _Point);
   ...
   if(priceChanged)
   {
      ...
      if(ui.orderType == UI_MARKET)
      {
         if(ObjectFind(0, g_prefix + "Edit_Price") >= 0)
         {
            ...
            ObjectSetInteger(0, g_prefix + "Edit_Price", OBJPROP_READONLY, true);
            ObjectSetInteger(0, g_prefix + "Edit_Price", OBJPROP_BGCOLOR,  C'40,40,40');
            ObjectSetInteger(0, g_prefix + "Edit_Price", OBJPROP_COLOR,    C'150,150,150');
         }
      ...
      else
      {
         if(ObjectFind(0, g_prefix + "Edit_Price") >= 0)
         {
            ObjectSetInteger(0, g_prefix + "Edit_Price", OBJPROP_READONLY, false);
            ObjectSetInteger(0, g_prefix + "Edit_Price", OBJPROP_BGCOLOR,  COLOR_EDIT);
            ObjectSetInteger(0, g_prefix + "Edit_Price", OBJPROP_COLOR,    COLOR_TEXT);
         }
         UpdateSLTPPrices(ui.customPrice);
         if(ui.isVisualizing) DrawVisualization();
      }

      double ref = (ui.orderType == UI_LIMIT) ? ui.customPrice : ((ui.side == UI_BUY) ? ask : bid);
      UpdateStats(ref);
   }
...
void LoadState()
{
   ...
   if(GlobalVariableCheck("TM3_" + id + "_Basket"))
      ui.basketEnabled = (bool)GlobalVariableGet("TM3_" + id + "_Basket");
}
...
int PanelHeight()
{
   return 440 + MathMax(ui.partialsCount, 0) * 18;
}
